const chalk = require("chalk");
const fse = require('fs-extra');
const fs = require("fs");
const path = require('path');
const toml = require('@iarna/toml');
const execSync = require('child_process').execSync;

exports.removeEntryPoint = function(name) {
    const root = process.cwd();
    const appPath = path.join(root, "./.bunbuild");

    console.log(chalk.bgCyanBright.black.bold("[Bun Build]")+chalk.bgBlack.blue.italic(" $ Add"));
    if (appPath == root) {
        return;
    }
    if (fse.existsSync(appPath)) {
        let rawBuildFile = toml.parse(fs.readFileSync(appPath));
        if (rawBuildFile["files"].find(x => x === name) != undefined) {
            rawBuildFile["files"] = rawBuildFile["files"].filter((item) => item !== name);
            fs.writeFileSync(appPath, "# !bunbuild | Do not edit this file manually; automatically generated.\n"+toml.stringify(rawBuildFile));
            console.log(chalk.bgCyanBright.black.bold("[Bun Build]")+chalk.bgBlack.greenBright(" Successfully Removed Entry Point from Bunbuild file"));
        } else {
            console.log(chalk.bgCyanBright.black.bold("[Bun Build]")+chalk.bgBlack.redBright(" Error | This entry point does not exist!"));
        }
    } else {
        console.log(chalk.bgCyanBright.black.bold("[Bun Build]")+chalk.bgBlack.redBright(" Error | Bunbuild file does not exist!"));
        process.exit(1);
    }
}